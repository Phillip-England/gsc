...
func Navbar(title string, subtext string, navmenu string) string {
	return fmt.Sprintf( /*html*/ `
		...
		<div id='navbar-menu' class='hidden relative z-30'>%s</div>
	...
	`, title, subtext, navmenu)
}

func Navmenu(navitems ...string) string {
	output := strings.Join(navitems, "\n")
	return fmt.Sprintf( /*html*/ `
...
func NavItem(title string, href string) string {
	return fmt.Sprintf( /*html*/ `
		<li class='flex w-full rounded border hover:bg-gray-100'>
			<a href='%s' class='p-4 w-full text-sm'>%s</a>
		</li>
	`, href, title)
}
